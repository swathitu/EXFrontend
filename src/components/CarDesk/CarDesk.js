...
const getItineraryDetails = (associatedData) => {
  if (associatedData.CarData && associatedData.CarData.length > 0) {
    ...
    return {
      ...
      agentRowId: car.AGENT_ID || null,
      agentEmail: car.AGENT_EMAIL || null,
      agentName: car.AGENT_NAME || null,
    };
  }
...
const CarDesk = () => {
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      try {
        ...
        const carRequests = rawData.filter(
          (trip) =>
            trip.associatedData &&
            trip.associatedData.CarData &&
  ...
  if (loading) return <h5>Loading Car Requests...</h5>;
  if (requests.length === 0) return <h5>No pending Car requests found.</h5>;
