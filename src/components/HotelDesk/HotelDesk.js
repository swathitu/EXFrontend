...
const getItineraryDetails = (associatedData) => {
  if (associatedData.HotelData && associatedData.HotelData.length > 0) {
    ...
    return {
      ...
      startDate: hotel.HOTEL_CHECKIN_DATE || "N/A",
      subtableRowId: hotel.ROWID,
      agentRowId: hotel.AGENT_ID || null,
      agentEmail: hotel.AGENT_EMAIL || null,
      agentName: hotel.AGENT_NAME || null,
    };
  }
...
const HotelDesk = () => {
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      try {
        ...
        // Filter for hotel requests only
        const hotelRequests = rawData.filter(
          (trip) =>
            trip.associatedData &&
            trip.associatedData.HotelData &&
            trip.associatedData.HotelData.length > 0
        );

        const processedData = hotelRequests.map((trip) => {
  ...
  if (loading) return <h5>Loading Hotel Requests...</h5>;
  if (requests.length === 0) return <h5>No pending Hotel requests found.</h5>;
              ...
              >
                ...
                <td>
                  <select
                    value={item.status}
                    onChange={(e) =>
                      handleDropdownChange(item.id, "status", e.target.value)
                    }
                  ...
                  >
                    ...
                    {AVAILABLE_ACTION_STATUSES.filter(
                      (o) => o !== item.status
                    ).map((option) => (
