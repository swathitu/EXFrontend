...
const HotelDesk = () => { 
    ...
    useEffect(() => {
        const fetchRequests = async () => {
            try {
                ...
                const hotelRequests = rawData.filter(trip => 
                    trip.associatedData && 
                    trip.associatedData.HotelData && // Check for CarData instead of BusData
    ...
    if (loading) {
        ...
        return <h5>Loading Hotel Requests...</h5>;
    }

    if (requests.length === 0) {
        // ⭐ MODIFIED: No requests message
        return <h5>No pending **Hotel** requests found.</h5>; 
    }
