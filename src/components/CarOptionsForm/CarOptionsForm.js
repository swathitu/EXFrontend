...
import './CarOptionsForm.css'; // Uses the shared CSS
...
export default function CarOptionsForm({ open, onClose, bookingData }) {
    ...
    useEffect(() => {
        if (bookingData) {
            setOptions([
                {
                    id: 1,
                    carrier: 'Dummy Car Rental',
                    depDate: getISODate(bookingData.pickUpDate),
                    depTime: '09:00',
                    arrDate: getISODate(bookingData.dropOffDate),
                    arrTime: '11:00',
                    carType: 'small', // Set to match screenshot
                    amount: '3500.00',
                    currency: 'INR',
                    isRefundable: false,
                    notes: '',
    ...
    const handleSave = () => {
        console.log("Saving Car Options:", options);
        ...
        <div className="modal-overlay" onClick={onClose}>
            <div className="modal-content large" onClick={e => e.stopPropagation()}>
                ...
                <div className="modal-subheader">
                    <span className="route">{bookingData.pickUpLocation} &rarr; {bookingData.dropOffLocation}</span>
                    <span className="date">{bookingData.pickUpDisplayDate} - {bookingData.dropOffDisplayDate}</span>
                </div>
                        ...
                        <div className="option-card" key={opt.id}>
                            ...
                            <div className="trip-option-form">
                                ...
                                <table className="trip-options-table">
                                    <thead>
                                        <tr>
                                            {/* --- MODIFIED: Using percentages --- */}
                                            <th style={{width: '28%'}}><span className="required">Car Name</span></th>
                                            <th style={{width: '28%'}}><span className="required">Departure Date & Time</span></th>
                                            <th style={{width: '28%'}}><span className="required">Arrival Date & Time</span></th>
                                            <th style={{width: '16%'}}><span>Car Type</span></th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <tr>
                                                ...
                                                <div className="date-time-cell-wrapper">
                                                    ...
                                                    <div className="location-row">
                                                        ...
                                                        <button className="btn-city-dropdown">
                                                            {bookingData.pickUpLocation} <CaretIcon />
                                                        </button>
                                                    </div>
                                                </div>
                                            </td>
                                            ...
                                            <td>
                                                <div className="date-time-cell-wrapper">
                                                    ...
                                                    <div className="location-row">
                                                        <span>Arrive at:</span>
                                                        <button className="btn-city-dropdown">
                                                            {bookingData.dropOffLocation} <CaretIcon />
                                                        </button>
                                                    </div>
                                                </div>
                                            </td>
                                            {/* --- MODIFIED: Car Type Dropdown --- */}
                                            <td>
                                                <select 
                                                    className="form-control"
                                                    value={opt.carType}
                                                    onChange={(e) => handleOptionChange(opt.id, 'carType', e.target.value)}
                                                >
                                ...
                                <div className="cost-details-row">
                                    <div className="form-group-amount">
                                        ...
                                        <div className="amount-group">
                                            <select className="currency-dropdown" value={opt.currency} onChange={(e) => handleOptionChange(opt.id, 'currency', e.target.value)}>
