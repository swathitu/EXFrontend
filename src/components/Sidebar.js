  ...
  */
  ...
  const MENU_ALL = [
    ...
    { key: "ta-trips", label: "TA-Trips", icon: "business", hasPlus: false },
  ...
  export default function Sidebar({
    ...
    if (role === "admin") {
      roleMenu = MENU_ALL.filter((m) =>
        ["home", "trip","ta-trips","masters", "expenseDataView", "travelDesk", 'trips'].includes(m.key)
      );
    } else if (role === "admin1") {
      roleMenu = MENU_ALL.filter((m) =>
      ...
      // Agent sees Home and TA-Trips
      roleMenu = MENU_ALL.filter((m) => ["ta-trips"].includes(m.key));
