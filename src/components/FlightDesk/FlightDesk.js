...
const getItineraryDetails = (associatedData) => {
  if (associatedData.FlightData && associatedData.FlightData.length > 0) {
    ...
    return {
      ...
      agentRowId: flight.AGENT_ID || null,
      agentEmail: flight.AGENT_EMAIL || null,
      agentName: flight.AGENT_NAME || null,
    };
  }
...
const FLIGHTDESK = () => {
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      try {
        ...
        const flightRequests = rawData.filter(
          (trip) =>
            trip.associatedData &&
            trip.associatedData.FlightData &&
            trip.associatedData.FlightData.length > 0
        );

        const processedData = flightRequests.map((trip) => {
  ...
  const handleDropdownChange = (requestId, field, value) => {
    setRequests((prevRequests) =>
      prevRequests.map((req) => (req.id === requestId ? { ...req, [field]: value } : req))
    );
    console.log(`Updated Request ID ${requestId} - ${field}: ${value}`);
  };

  if (loading) return <h5>Loading Flight Requests...</h5>;
  if (requests.length === 0) return <h5>No pending Flight requests found.</h5>;

  return (
    <div className="all-requests-container">
      <h3>Flight Requests ✈️</h3>
