...
const FLIGHTDESK = () => { 
    ...
    useEffect(() => {
        const fetchRequests = async () => {
            try {
                ...
                const flightRequests = rawData.filter(trip => 
                    trip.associatedData && 
                    trip.associatedData.FlightData && // Check for CarData instead of BusData
    ...
    if (loading) {
        ...
        return <h5>Loading Flight Requests...</h5>;
    }

    if (requests.length === 0) {
        // ⭐ MODIFIED: No requests message
        return <h5>No pending **Flight** requests found.</h5>; 
    }
