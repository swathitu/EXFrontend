...
const App = () => {
  ...
  useEffect(() => {
    if (selectedMode) {
      ...
      const filteredData = ticketData.filter(
        (item) => item.TypeOfMode === selectedMode && item.status?.toLowerCase() === selectedStatus
      );
  ...
  const handleEditClick = (row) => {
    ...
    setEditingAmount(row.Amount !== undefined && row.Amount !== null ? row.Amount.toString() : '');
    setEditingBillNumber(row.BillNumber !== undefined && row.BillNumber !== null ? row.BillNumber.toString() : '');
...
const handleUpdateVendor = async (bookingId) => {
  ...
  const getDetailedColumns = (data, mode) => {
    ...
    const secondaryFields = ["Amount", "vendorName", "BillNumber", "BillDate", "TypeOfBook"];
    ...
    keys.forEach(key => {
        ...
        if (key.toLowerCase().startsWith(mode.toLowerCase()) && 
  ...
  return (
    <div className="container">
      <div className="dashboard-card">
        ...
        {!loading && !error && !selectedMode && Object.keys(aggregatedData).length === 0 && (
          <div className="loading-container">
              ...
              <tbody>
                {Object.entries(aggregatedData).map(([mode, counts]) => (
                  <tr key={mode} className="table-row">
                    <td className="table-cell left-round">
                      {mode}
                    </td>
                    <td className="table-cell">
                      <span className="status-pending" onClick={() => handlePendingClick(mode, counts.pending)}>
                        {counts.pending}
                      </span>
                    </td>
                    <td className="table-cell right-round">
                      <span className="status-completed" onClick={() => handleCompletedClick(mode, counts.completed)}>
                        {counts.completed}
                      </span>
                    </td>
                  </tr>
        ...
        {!loading && !error && selectedMode && (
          ...
          <div className="table-container1">
            <h2 className="detailed-title">{selectedStatus.charAt(0).toUpperCase() + selectedStatus.slice(1)} {selectedMode.charAt(0).toUpperCase() + selectedMode.slice(1)} Tickets</h2>
            <table className="detailed-table">
              <thead className="table-header-row">
                <tr>
                  {detailedColumns.map(column => (
                    <th key={column.key} scope="col" className="detailed-table-header-cell">
                      {column.label}
                    </th>
                  ))}
                  {selectedStatus === "pending" && (
                    <th scope="col" className="detailed-table-header-cell">Actions</th>
                  )}
                </tr>
              </thead>
              <tbody className="detailed-table-body">
                {detailedData.map((row) => (
                  <tr key={row.bookingId} className="detailed-row">
                    {detailedColumns.map(column => {
                      const isEditable = ["vendorName", "Amount", "BillNumber", "BillDate", "TypeOfBook", "status"].includes(column.key);
                      ...
                      if (isEditing && isEditable) {
                            cellContent = (
                              <input
                                type={column.key === 'Amount' || column.key === 'BillNumber' ? 'number' : column.key === 'BillDate' ? 'date' : 'text'}
                                value={
                                  column.key === "vendorName" ? editingVendorName :
                                  column.key === "Amount" ? editingAmount :
                                  column.key === "BillNumber" ? editingBillNumber :
                                  column.key === "BillDate" ? editingBillDate :
                                  column.key === "TypeOfBook" ? editingTypeOfBook : ''
                                }
                                onChange={(e) => {
                                  if (column.key === "vendorName") setEditingVendorName(e.target.value);
                                  else if (column.key === "Amount") setEditingAmount(e.target.value);
                                  else if (column.key === "BillNumber") setEditingBillNumber(e.target.value);
                                  else if (column.key === "BillDate") setEditingBillDate(e.target.value);
                                  else if (column.key === "TypeOfBook") setEditingTypeOfBook(e.target.value);
                                }}
                            ...
                            cellContent = row[column.key] !== undefined && row[column.key] !== null ? row[column.key] : "N/A";
                          }
